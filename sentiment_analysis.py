...
nltk_stop_words.append('vou')
nltk_stop_words.append('tão')
nltk_stop_words.append('vai')

def removeStopWords(texto): #remove elementos da sentença que não possuam valor semântico
    frases = []
    for (palavras, emocao) in texto:
        senstop = [p for p in palavras.split() if p not in nltk_stop_words]
        frases.append((senstop, emocao))
    return frases

def applyStemming(texto): #reduz frases ao seu radical
    stemmer = nltk.stem.RSLPStemmer() #steemer usado especificamente para o português
    frasesStemming = []

    for (palavras, emocao) in texto:
        hasStemming = [str(stemmer.stem(p)) for p in palavras.split() if p not in nltk_stop_words]
        frasesStemming.append((hasStemming, emocao))

    return frasesStemming

def searchWords(texto): #procura por uma palavra em específico
    allWords = []
...
def searchFrequency(words): #retorna a frequência em que as palavras estão distribuídas
    words = nltk.FreqDist(words)

    return words

def searchUniqWords(frequency_words): #usada para verificar palavras únicas e n duplicadas
    return frequency_words.keys()

def extractWords(documento): #verifica se uma palavra existe ou não em uma frase/documento
    doc = set(documento)
    caracteristicas = {}
...
#aula 32
fraseComStemmingTreinamento = applyStemming(basetreinamento)
fraseComStemmingTeste = applyStemming(baseteste)

palavrasTreinamento = searchWords(fraseComStemmingTreinamento)
palavrasTeste = searchWords(fraseComStemmingTeste)

frequenciaTreinamento = searchFrequency(palavrasTreinamento)
frequenciaTeste = searchFrequency(palavrasTeste)

palavrasUnicasTreinamento = searchUniqWords(frequenciaTreinamento)
palavrasUnicasTeste = searchUniqWords(frequenciaTeste)

baseCompletaTreinamento = nltk.classify.apply_features(extractWords, fraseComStemmingTreinamento)
baseCompletaTeste = nltk.classify.apply_features(extractWords, fraseComStemmingTeste)
